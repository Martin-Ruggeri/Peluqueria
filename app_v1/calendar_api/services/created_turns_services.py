...
def save_turns(turns):
  ...
  for turn in turns:
    try:
      ...
      msj = f'el turn {turn}, no se ha podido guardar correctamente'
      print(msj)
  return turns_save
...
def validate_created_turn(schedule: Schedule, detail_calendar: DetailCalendar) -> bool:
  ...
  try:
    ...
    print('No existe detail calendar')
    return False
  
  # Validar que el dia sea futuro (actual)
  if (detail_calendar.date < date.today()):
    pritn(f'El dia {detail_calendar.date}, ya ah pasado')
    return False
  
  # Validar si el dia esta incluido en uno de los dias del schedule
  ...
  if (not make_detail_calendar):
    ...
    print(f'El dia {day_str}, no esta en la configuracion del horario')
    return False
  ...
  if (len(turns) > 0):
    print(f'Ya existen turnos creados para el dia {detail_calendar.date} y horario {schedule}')
    return False
